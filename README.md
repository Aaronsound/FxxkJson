...
- 格式化和编辑保存保留长整数、高精度小数及重复字段；重复 key 会提示，结构对比遇到歧义时暂停。
- 按差异类型和字段路径筛选对比结果，并继续加载后续差异。
- 通过“更多 → 另存为”导出原文、格式化或单行压缩 JSON。
- 通过“更多 → 恢复关闭的标签”或 `Ctrl/Cmd + Shift + T` 恢复本次运行中误关的标签（有数量和内存上限，不自动落盘）。
