...
- 对比遇到重复 key 会显示具体哪一侧有问题，并暂停结构对比；请先使用对应标签的“编辑 JSON”处理重复字段，不会自动删除字段。
- 对比结果可以按新增、删除、修改筛选，也可以按字段路径包含的文字筛选（区分大小写、不是正则表达式）。筛选针对已经加载的差异；只要提示尚未完成，即使当前匹配为零，也可以继续加载。清除筛选可恢复查看全部已加载结果。
- 误关闭标签后，通过“更多 → 恢复关闭的标签”或 `Ctrl + Shift + T`（macOS 为 `Cmd + Shift + T`）恢复名称和原文，包括语法错误的草稿。查看器会重新生成，不恢复撤销记录、搜索或折叠状态。
- 恢复记录只在当前运行中存在，不自动落盘；最多 10 个非空标签，总计 128 MiB 文本内存（按 UTF-16 保守计算，约 6,700 万字符）。超出预算时淘汰最早记录；单个标签超过预算则不保留，并清空旧恢复记录，避免误恢复更早文件。退出应用后记录消失，重要修改请先另存为。

## 无损格式化与另存为（v1.0.37 起）

格式化保留长整数、高精度小数、指数写法及重复 key，不将它们转换成普通 JavaScript 数字后重新输出。发现重复 key 时，状态区会提示并可定位原文中的第一处重复；对于转义字符串内部的重复字段，只提示，不提供不准确的原文位置。重复字段请使用顶部“编辑 JSON”处理，同名节点的单独修改会被阻止。

使用“更多 → 另存为…”选择左侧原文、格式化 JSON 或单行压缩 JSON，再选择保存位置。原文模式允许保存仍有语法错误的草稿，另外两种模式要求内容可解析。默认建议新文件名，不改变当前标签；取消系统保存窗口不会关闭选择内容的弹窗。若主动选择已有文件，请确认系统的覆盖提示。
...
如果格式化遇到可定位的语法错误，提示区会显示行、列及“定位错误”。点击后左侧会滚动并短暂高亮对应内容，不会修改原文，也不会自动修复。修改内容后旧位置失效，重新格式化会更新提示。位置表示解析器发现异常的地方，根因可能在前面；文档末尾缺少括号等情况会标记末尾附近。

大文件左侧查看器不能直接输入时，点击原有的 **编辑 JSON** 即可打开错误原文，并跳到错误附近，不需要先修复成功。修改后点击 **更新为原始 JSON**：校验通过才会更新原文；仍有错误时保留草稿并在弹窗内定位，继续修改即可。取消不会修改原文。原有 **修复 JSON** 仍用于自动修复。
...
搜索结果会分批加载；匹配很多时可以点击“加载更多”。点击上一个或下一个匹配项不会清除已加载结果。快速连续输入时，以最新查询为准。
...
6. 差异超过 2,000 处时，点击“继续加载”逐批查看剩余差异；用“上一批”“下一批”查看已加载结果。加载期间的统计仅包含已加载差异，显示“已完成全部对比”后才是最终总数。
   列表按滚动位置轻量显示，不会丢弃未显示的差异；查看完整值后点击“返回差异列表”，会回到刚才的位置。支持用 Tab / Shift+Tab 连续切换差异查看按钮。
7. 点击差异路径下的“查看完整值”，可查看两侧的完整 JSON 值。长内容按段阅读，也可跳到最后一段；“复制完整值”复制整段值，而非当前可见部分。数字保留精度，`1`、`1.0`、`1e0` 仍视为相同数值。
   长值按需读取片段；返回差异列表会释放详情读取缓存，不影响已加载的差异，重新打开仍可继续查看。
