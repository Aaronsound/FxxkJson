...
export function getCompactPathLabel(pathText: string) {
  return pathText.length > MAX_HEADER_PATH_LENGTH ? `${pathText.slice(0, MAX_HEADER_PATH_LENGTH - 3)}...` : pathText;
}
...
export function useRightSearchQuickAccess(activeTabId: string | null) {
  ...
  const activeRightPinnedPaths = activeTabId ? (rightPinnedPathsByTab[activeTabId] ?? []) : [];
  const activeRightPinnedPathItems = useMemo(
    () =>
      activeRightPinnedPaths.map((item) => ({
        id: item.id,
  ...
  const getPinnedPath = useCallback(
    (tabId: string, id: string) => (rightPinnedPathsByTab[tabId] ?? []).find((item) => item.id === id) ?? null,
